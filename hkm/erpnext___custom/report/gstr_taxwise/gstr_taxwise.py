		...
		for slab in GST_SLABS:
			...
			for gt in GST_TYPES:
				...
				if head in invoices[inv]['taxes']:
					...
					row[head.split("-")[0]+'_amount'] = invoices[inv]['taxes'][head]['tax']
...
def get_conditions(filters):
	"""Get Filter Items"""
	conditions=""
	for opts in (("company", " and sinv.company=%(company)s"),
	...
	for (invoice, account_head, item_wise_tax_detail) in tax_details:
		...
		for item in tax_detail:
			...
			if tax_detail[item][1] >0:
				invoices_data[invoice][item][gstr(gst_type,tax_detail[item][0])] = tax_detail[item][1]
...
def get_invoices(filters):
	...
	invoices_data = frappe.db.sql(
		...
		where sinv.docstatus = 1 %s order by posting_date desc, sinv.name desc"""%(conditions),
		filters,
		as_dict=0,
	)

	inv_item_tax_map = get_invoice_item_tax_map(list(set([inv[0] for inv in invoices_data])))

	invoices = {}
	
	for (invoice_no, customer_name, posting_date, taxes_and_charges, item_code,item_name,gst_hsn_code,taxable,item_tax_template, tax_rate, tax) in invoices_data:

		if invoice_no not in invoices:
			invoices.setdefault(invoice_no,{
											"details"	:	{
		...
		TAXABLE = taxable
		item_taxes = inv_item_tax_map[invoice_no][item_name if item_code is None else item_code]

		#Tax Free Item
		if not item_taxes:
			invoices[invoice_no]["taxes"].setdefault('IGST-0.0',{
																			'hsns' : [],
																			'taxable' : 0,
																			'tax' : 0,
																			'tax_rate' : 0
		...
		for gtype in item_taxes:
			if gtype not in invoices[invoice_no]["taxes"]:
				invoices[invoice_no]["taxes"].setdefault(gtype,{
																			'hsns' : [],
																			'taxable' : 0,
																			'tax' : 0,
																			'tax_rate' : 0
...
def get_columns(filters):
	...
	columns = [
		{"label": _("Invoice"),"fieldname": "invoice","fieldtype": "Link","options": "Sales Invoice","width": 120,},
	...
	tax_columns = [
		{"label": _("CGST Rate"),"fieldname": "CGST_rate","fieldtype": "Percent","width": 120},
		{"label": _("CGST Amount"),"fieldname": "CGST_amount","fieldtype": "Currency","options": "currency","width": 120},
		{"label": _("SGST Rate"),"fieldname": "SGST_rate","fieldtype": "Percent","width": 120},
		{"label": _("SGST Amount"),"fieldname": "SGST_amount","fieldtype": "Currency","options": "currency","width": 120},
		{"label": _("IGST Rate"),"fieldname": "IGST_rate","fieldtype": "Percent","width": 120},
		{"label": _("IGST Amount"),"fieldname": "IGST_amount","fieldtype": "Currency","options": "currency","width": 120},
	]

	total_columns = [
		{"label": _("Total Tax"),"fieldname": "total_tax","fieldtype": "Currency","options": "currency","width": 120},
		{"label": _("Taxable"),"fieldname": "taxable_amount","fieldtype": "Currency","options": "currency","width": 120},
