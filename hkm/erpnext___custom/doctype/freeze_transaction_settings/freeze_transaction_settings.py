	...
	if not company_frozen_document:
		if frappe.db.exists('Freeze Transaction Settings', {"company":company}):
			...
			for d in doc.get("forzen_documents"):
				...
				company_frozen_document.setdefault(d.document_type, frappe._dict(
...
def get_frozen_document_list():
	frozen_document_list = frappe.cache().hget("frozen_document_list", None) or frappe._dict()
	if not company_frozen_document:
		frozen_document_list = list(set(frappe.get_all('Freeze Transaction Settings Document', pluck='document_type')))
		frappe.cache().hset("frozen_document_list", None, frozen_document_list)

	return frozen_document_list
	...
	if transaction_date and company:
		...
		if company_frozen_document:
			...
			if not is_valid_date and frappe.session.user not in frozen_tran_modifier:
				...
				frappe.throw(_("<b class='text-danger'>{0}<br>You are not authorized to add/cancel <b>{1}</b> of {2}.").format(message, doc.doctype, formatdate(transaction_date)))
