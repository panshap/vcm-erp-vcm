...
def get_alm(doc):
	...
	alms = frappe.db.sql("""
						...
						""".format(doc.doctype,doc.company),as_dict=1)
	if len(alms) != 0:
		alm = alms[0]
		alm_levels = frappe.db.sql("""
						...
						""".format(alm['name'],doc.department),as_dict=1)
		if len(alm_levels)>0:
			...
			for level in alm_levels:
				if level['expense_type'] == 'ANY':
					...
					if eval(str(deciding_amount)+level['amount_condition']) and doc_expense_type == level['expense_type']:
						final_alm_level = level
						break
...
def remove_all_assignments(doc):
	frappe.db.sql("""
		...
		AND owner = '{}'
		""".format(doc.name,frappe.session.user))
	frappe.db.commit()
	return