...
def before_save(self,method=None):
	...
	item_names = item_names[0:39]+"..." if len(item_names)>40 else item_names[0:39]
...
def validate(self, method=None):
	...
	validate_purchase_item(self)
	validate_asset_item(self)
	validate_work_head(self)

def on_submit(self, method=None):
	set_boq_status(self)
	notify_mrn(self,method)

def validate_work_head(doc):
	if doc.work_head:
	...
	return
def set_boq_status(self, method=None):
	if self.boq and frappe.db.exists("BOQ", self.boq):
		boq = frappe.get_doc("BOQ", self.boq)
		boq.set_status(update=True)


def validate_work_order_item(self, method=None):
	
	if not self.meta.get_field("for_a_work_order") or not self.for_a_work_order: 
		return

	if not frappe.db.get_single_value('HKM ERPNext Settings', 'validate_work_order_item'):
		return
...
def check_items_are_not_from_template(self, method=None):
	for item in self.items:
...
def validate_asset_item(self):
	if not self.material_request_type == 'Material Issue':
		return
	invalid_items = []
	asset_items = frappe.get_list('Item', pluck='name', filters={'disabled':0, 'is_fixed_asset':1})
	for d in self.get("items"):
		if d.item_code in asset_items:
			invalid_items.append(d)
	for d in invalid_items:
		frappe.throw(
			_('Row#{0}: Asset Item {1} is not allowed for MRN of Issue.<br> Please select a stock item.')
			.format(
				d.idx,
...
def validate_purchase_item(self):
	...
	for d in self.get("items"):
		...
		if item.is_stock_item and not purchase_for_stock:
			frappe.throw(
				_('Row#{0}: {1} is a Stock Item. So you can\'t make a Purchase MRN for that. It should be routed through Store<br>It is allowed only for Role : {2}')
				.format(
					d.idx,
					frappe.bold(d.item_name),
