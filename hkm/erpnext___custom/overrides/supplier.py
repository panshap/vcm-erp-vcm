...
def success_mail(supplier,sc_request):
	...
	message = """
				...
				<p>Thanks,</p>
				<p>ERP Team</p>
				""".format(supplier.supplier_name,frappe.utils.get_url_to_form("Supplier", supplier.name),time_taken,postfix,supplier.owner)
	return message


def creation_from_gstin(self,method):
	if self.is_new() and not self.get('gstin') and not 'Item & Supplier Creator' in frappe.get_roles(frappe.session.user):
